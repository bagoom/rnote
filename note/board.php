...
if (isset($write['wr_is_comment']) && $write['wr_is_comment']) {
    goto_url('./board.php?company='.$company.'&amp;wr_id='.$write['wr_parent'].'#c_'.$wr_id);
}

if (!$company) {
    $msg = "company 값이 넘어오지 않았습니다.\\n\\nboard.php?company=code 와 같은 방식으로 넘겨 주세요.";
    alert($msg);
}
// 그룹접근 사용
if (isset($group['gr_use_access']) && $group['gr_use_access']) {
    if ($is_guest) {
        ...
        alert($msg, './login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(G5_NOTE_URL.'/board.php?company='.$company.'&amp;wr_id='.$wr_id.$qstr));
    }
...
if (isset($wr_id) && $wr_id) {
    ...
    if (!$write['wr_id']) {
        $msg = '글이 존재하지 않습니다.\\n\\n글이 삭제되었거나 이동된 경우입니다.';
        alert($msg, './board.php?company='.$company);
    }
    ...
    if ($member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            ...
            alert('글을 읽을 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', './login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(G5_NOTE_URL.'/board.php?company='.$company.'&amp;wr_id='.$wr_id.$qstr));
    }
    ...
    if (($write['mb_id'] && $write['mb_id'] == $member['mb_id']) || $is_admin) {
        ...
        if (strstr($write['wr_option'], "secret"))
        {
            ...
            $ss_name = 'ss_secret_'.$company.'_'.$write['wr_num'];

            if (!$is_owner)
            {
                //$ss_name = "ss_secret_{$company}_{$wr_id}";
                // 한번 읽은 게시물의 번호는 세션에 저장되어 있고 같은 게시물을 읽을 경우는 다시 비밀번호를 묻지 않습니다.
                // 이 게시물이 저장된 게시물이 아니면서 관리자가 아니라면
                //if ("$company|$write['wr_num']" != get_session("ss_secret"))
                if (!get_session($ss_name))
                    goto_url('./password.php?w=s&amp;company='.$company.'&amp;wr_id='.$wr_id.$qstr);
            }
    ...
    $ss_name = 'ss_view_'.$company.'_'.$wr_id;
    if (!get_session($ss_name))
    {
        ...
        if ($write['mb_id'] && $write['mb_id'] == $member['mb_id']) {
            ...
            insert_point($member['mb_id'], $board['bo_read_point'], ((G5_IS_MOBILE && $board['bo_mobile_subject']) ? $board['bo_mobile_subject'] : $board['bo_subject']).' '.$wr_id.' 글읽기', $company, $wr_id, '읽기');
        }
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
            ...
            alert('목록을 볼 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', './login.php?'.$qstr.'&url='.urlencode(G5_NOTE_URL.'/board.php?company='.$company.($qstr?'&amp;':'')));
    }
...
if ($member['mb_id'] && ($is_admin == 'super' || $group['gr_admin'] == $member['mb_id']))
    $admin_href = G5_ADMIN_URL.'/board_form.php?w=u&amp;company='.$company;

include_once(G5_NOTE_PATH.'/board_head.php');
...
if (isset($wr_id) && $wr_id) {
    include_once(G5_NOTE_PATH.'/view.php');
}

// 전체목록보이기 사용이 "예" 또는 wr_id 값이 없다면 목록을 보임
//if ($board['bo_use_list_view'] || empty($wr_id))
if ($member['mb_level'] >= $board['bo_list_level'] && $board['bo_use_list_view'] || empty($wr_id))
    include_once (G5_NOTE_PATH.'/list.php');

// include_once(G5_NOTE_PATH.'/board_tail.php');
