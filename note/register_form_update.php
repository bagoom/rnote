...
if ($w == '') {
    ...
    if ($config['cf_email_mb_member']) {
        ...
        if ($config['cf_use_email_certify']) {
            ...
            $certify_href = G5_NOTE_URL.'/email_certify.php?mb_id='.$mb_id.'&amp;mb_md5='.$mb_md5;
        }
...
if ($w == '') {
    goto_url(G5_HTTP_NOTE_URL.'/register_result.php');
} else if ($w == 'u') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        <title>회원정보수정</title>
        <body>
        <form name="fregisterupdate" method="post" action="'.G5_HTTP_NOTE_URL.'/register_form.php">
