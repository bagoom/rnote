...
if ($w == '') {
   ...
   $sql2 = " insert into g5_group
                set gr_id = '{$mb_id}',
                gr_subject = '$group_name',
                gr_admin = '{$mb_id}',
                gr_nick = '{$mb_name}',
                gr_device = 'both',
                gr_use_access = '1',
    ...
    // 이름변경시 매물 작성자의 이름도 함께 수정
    $write_table = "g5_write_".$member[mb_id];
    sql_query("update {$write_table}
                set wr_writer = '{$mb_name}' where wr_writer = '{$mb_prev_name}' ");
...
if ($w == '') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        <title>회원정보수정</title>
        <body>
        <form name="fregisterupdate" method="post" action="'.G5_HTTP_BBS_URL.'/register_form_default.php">
