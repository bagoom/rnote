...
function get_sql_search3($search_ca_name, $search_field, $search_text, $search_operator='and',$search_arr='')
{
    ...
    $rent_deposit_min = $_POST["wr_rent_deposit_min"];
    $rent_deposit_max = $_POST["wr_rent_deposit_max"];
    if ($search_ca_name)
    ...
    if($_POST["wr_m_rate_min"] && $_POST["wr_m_rate_max"]) {
       $str = append_sql2($str, " and ", " wr_m_rate between  '$_POST[wr_m_rate_min]' and  '$_POST[wr_m_rate_max]'");
     }elseif ($_POST["wr_m_rate_min"] || $_POST["wr_m_rate_max"]){
       $str = append_sql2($str, " and ", " wr_m_rate <= '$_POST[wr_m_rate_min]' ");
     }
     if($_POST["wr_rent_deposit_min"] && $_POST["wr_rent_deposit_max"]) {
        $str = append_sql2($str, " and ", " wr_rent_deposit between $rent_deposit_min and $rent_deposit_max");
      }elseif ($_POST["wr_rent_deposit_min"] || $_POST["wr_rent_deposit_max"]){
        $str = append_sql2($str, " and ", " wr_rent_deposit <= '$_POST[wr_rent_deposit_min]' ");
      }
      if($_POST["wr_floor_min"] && $_POST["wr_floor_max"]) {
         $str = append_sql2($str, " and ", " wr_floor between '$_POST[wr_floor_min]' and '$_POST[wr_floor_max]'");
       }elseif ($_POST["wr_floor_min"] || $_POST["wr_floor_max"]){
         echo ($_POST["wr_floor_min"]);
         $str = append_sql2($str, " and ", " wr_floor <= '$_POST[wr_floor_min]' ");
       }
       if($_POST["wr_area_p_min"] && $_POST["wr_area_p_max"]) {
          $str = append_sql2($str, " and ", " wr_area_p between '$_POST[wr_area_p_min]' and '$_POST[wr_area_p_max]'");
        }elseif ($_POST["wr_area_p_min"] || $_POST["wr_area_p_max"]){
          $str = append_sql2($str, " and ", " wr_area_p <= '$_POST[wr_area_p_min]' ");
        }
        if($_POST["wr_premium_o_min"] && $_POST["wr_premium_o_max"]) {
           $str = append_sql2($str, " and ", " wr_premium_o between '$_POST[wr_premium_o_min]' and '$_POST[wr_premium_o_max]'");
         }elseif ($_POST["wr_premium_o_min"] || $_POST["wr_premium_o_max"]){
           $str = append_sql2($str, " and ", " wr_premium_o <= '$_POST[wr_premium_o_min]' ");
         }
...
$arr_search = array();
  if(($_POST["wr_subject"])) {
    ...
    $arr_search[val][] = $_POST["wr_subject"];
  }

	if(($_POST["wr_area_p"])) {
		$arr_search[name][] = "wr_area_p";
		$arr_search[val][] = $_POST["wr_area_p"];
	}
  if(($_POST["wr_sale_area"])) {
    $arr_search[name][] = "wr_sale_area";
    $arr_search[val][] = $_POST["wr_sale_area"];
  }

	if(($_POST["wr_floor"])) {
		$arr_search[name][] = "wr_floor";
		$arr_search[val][] = $_POST["wr_floor"];
	}
	if(($_POST["wr_rec_sectors"])) {
		$arr_search[name][] = "wr_rec_sectors";
		$arr_search[val][] = $_POST["wr_rec_sectors"];
	}
...
// print_r ($_POST);
	...
	if(($_GET["wr_code"])) {
		$arr_search[name][] = "wr_code";
		$arr_search[val][] = $_GET["wr_code"];
	}
	if(($_POST["wr_rent_deposit"])) {
		$arr_search[name][] = "wr_rent_deposit";
		$arr_search[val][] = $_POST["wr_rent_deposit"];
	}
  if(($_POST["wr_premium_o"])) {
		$arr_search[name][] = "wr_premium_o";
		$arr_search[val][] = $_POST["wr_premium_o"];
	}
  if(($_POST["wr_sale_area"])) {
		$arr_search[name][] = "wr_sale_area";
		$arr_search[val][] = $_POST["wr_sale_area"];
  ...
  if(($_POST["wr_address"])) {
		...
		$arr_search[val][] = $_POST["wr_address"];
	}
...
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.$qstr.'&board_list='.$board_list.'&wr_important='.$wr_important.'&wr_sale_type='.$wr_sale_type.'&wr_office_permission=2&amp;page=');
}

else if($wr_office_permission == '1'){
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.$qstr.'&office_write=1&board_list='.$board_list.'&wr_sale_type='.$wr_sale_type.'&wr_office_permission=1&amp;page=');
}

else if($gr_admin && $wr_important == '2'){
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.$qstr.'&board_list='.$board_list.'&wr_sale_type='.$wr_sale_type.'&wr_important=2&wr_writer='.$member[mb_name].'&amp;page=');
}

else{
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.$qstr.'&board_list='.$board_list.'&wr_important='.$wr_important.'&wr_sale_type='.$wr_sale_type.'&amp;page=');
}
$list_href = '';
$prev_part_href = '';
$next_part_href = '';
